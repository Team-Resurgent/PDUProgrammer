...
uint8_t STUSB4500::begin(uint8_t deviceAddress, i2c_inst_t *wirePort)
{
  readSectors = 0;
  _deviceAddress = deviceAddress; // If provided, store the I2C address from user
  _i2cPort = wirePort;            // Grab which port the user wants us to use

  uint8_t dummy_data = 0; // or whatever you'd send
  int result = i2c_write_blocking(_i2cPort, deviceAddress, &dummy_data, 1, false);
...
uint8_t STUSB4500::I2C_Write_USB_PD(uint16_t Register, uint8_t *DataW, uint16_t length)
{
  ...
  int result = i2c_write_blocking(_i2cPort, _deviceAddress, buffer, length + 2, false);
  sleep_ms(1);
...
{
  uint8_t reg_buf[2] = {(uint8_t)((Register >> 8) & 0xFF), (uint8_t)(Register & 0xFF)};
  int result = i2c_write_blocking(_i2cPort, _deviceAddress, reg_buf, 2, true);
  if (result == PICO_ERROR_GENERIC)
  {
    return 1;
  }
  result = i2c_read_blocking(_i2cPort, _deviceAddress, DataR, length, false);
  if (result == PICO_ERROR_GENERIC)
  {
    return 2;
  }
